...
func UseWithMigration(router gin.IRouter, mongoURL, migrationDir string, failOnOrderMismatch bool) gin.IRoutes {
	...
	if err := migrator.Migrate(migrationDir, db, failOnOrderMismatch); err != nil {
		log.Get().Fatalf("An error occurred while migrating data. %s", err.Error())
	}
...
// GetDb gets a clone of the DB session generated for the current gin.Context.
// It is important to close the session. See usage below.
//
//   Usage:
//      s, db := GetDb()
//      defer s.Close()
